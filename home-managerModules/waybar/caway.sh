...
playerctl -a metadata --format '{"text": "{{artist}} - {{title}}", "tooltip": "{{markup_escape(artist)}} - {{markup_escape(title)}}", "alt": "{{status}}", "class": "{{status}}"}' -F >$playerctl_waybar_pipe &
...
while read -r line; do
	...
	if [[ $(echo $line | jq -r '.class') == 'Playing' ]]; then
	    echo "$line" | jq --arg a $(echo $cmd2 | sed "$dict") '.text = $a' --unbuffered --compact-output
	elif [[ $(echo $line | jq -r '.class') == 'Paused' ]]; then
    ...
    if [[ $(echo $line | jq -r '.class') == 'Stopped' ]]; then 
	echo '{"text":"","tooltip":"nothing playing :o","alt":"Stopped","class":"Stopped"}' | jq --unbuffered --compact-output
    fi
